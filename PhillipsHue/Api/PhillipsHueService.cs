...
namespace PhillipsHue.Api
{
    ...
    [Route("/GetLightNames", "GET")]
    public class GetLightNames : IReturn<string>
    {
        [ApiMember(Name = "LightIds", Description = "Array of Light ids", IsRequired = true, DataType = "string", ParameterType = "query", Verb = "GET")]
    ...
    public class PhillipsHueService : IService
    {
        ...
        public string Get(GetLightNames request)
        {
            ...
            var json = new StreamReader(httpClient.Get(new HttpRequestOptions()
            {
                ...
                Url = (config.IsSecureConnection ? "https://" : "http://") + ip + "/api/" + userToken + "/lights",
                RequestContentType = "application/json"

            }).Result).ReadToEnd();

            logger.Info("HUE LIGHT API: " + json);

            return jsonSerializer.SerializeToString(new GetLightNames() { Lights = json });
        }
